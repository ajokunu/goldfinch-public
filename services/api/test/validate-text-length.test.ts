...
describe('free-text max-length validation', () => {
  ...
  describe('PATCH /accounts/{accountId} nameOverride + institutionOverride', () => {
    function patchEvent(body: Record<string, unknown>) {
      return makeEvent({
        routeKey: 'PATCH /accounts/{accountId}',
        pathParameters: { accountId: 'a1' },
        body,
      });
    }

    // The overrides reuse the account name/institution bounds.
    it(`accepts a nameOverride at the ${MAX_TEXT_LENGTHS.accountName}-character limit (200)`, async () => {
      ddbMock.on(GetCommand).resolves({ Item: makeAccountItem({ SK: 'ACCT#a1' }) });
      ddbMock.on(UpdateCommand).resolves({
        Attributes: makeAccountItem({
          SK: 'ACCT#a1',
          nameOverride: chars(MAX_TEXT_LENGTHS.accountName),
        }),
      });
      const res = await handler(
        patchEvent({ nameOverride: chars(MAX_TEXT_LENGTHS.accountName) }),
      );
      expect(res.statusCode).toBe(200);
    });

    it('rejects a nameOverride over the limit (400) before writing', async () => {
      ddbMock.on(GetCommand).resolves({ Item: makeAccountItem({ SK: 'ACCT#a1' }) });
      const res = await handler(
        patchEvent({ nameOverride: chars(MAX_TEXT_LENGTHS.accountName + 1) }),
      );
      expectValidationError(res);
      expect(ddbMock.commandCalls(UpdateCommand)).toHaveLength(0);
    });

    it('measures the TRIMMED nameOverride, so whitespace does not push it over', async () => {
      ddbMock.on(GetCommand).resolves({ Item: makeAccountItem({ SK: 'ACCT#a1' }) });
      ddbMock.on(UpdateCommand).resolves({
        Attributes: makeAccountItem({ SK: 'ACCT#a1' }),
      });
      const res = await handler(
        patchEvent({ nameOverride: `   ${chars(MAX_TEXT_LENGTHS.accountName)}   ` }),
      );
    ...
    it('rejects an institutionOverride over the limit (400) before writing', async () => {
      ddbMock.on(GetCommand).resolves({ Item: makeAccountItem({ SK: 'ACCT#a1' }) });
      const res = await handler(
        patchEvent({
          institutionOverride: chars(MAX_TEXT_LENGTHS.accountInstitution + 1),
        }),
      );
      expectValidationError(res);
      expect(ddbMock.commandCalls(UpdateCommand)).toHaveLength(0);
