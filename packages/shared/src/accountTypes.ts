...
/** Minimal slice for name precedence (full AccountItem/AccountDto satisfy it). */
export interface AccountNameFields {
  /** Synced/stored display name (required on every AccountItem). */
  name: string;
  /** USER-OWNED label override; absent/empty == none. */
  nameOverride?: string;
}

/**
 * THE effective account display name: `nameOverride ?? synced name`. An empty
 * or whitespace-only override is treated as no override (degrades to the synced
 * name) so dirty/cleared data never renders a blank label. Sole precedence
 * source — never inline `item.nameOverride ?? item.name` elsewhere.
 */
export function effectiveAccountName(item: AccountNameFields): string {
  const override = item.nameOverride;
...
/** Minimal slice for institution precedence (full AccountItem/AccountDto satisfy it). */
export interface AccountInstitutionFields {
  /** Synced/stored institution label (required on every AccountItem). */
  institution: string;
  /** USER-OWNED institution override; absent/empty == none. */
  institutionOverride?: string;
}

/**
 * THE effective institution/bank label: `institutionOverride ?? synced`. Empty
 * or whitespace-only override degrades to the synced value. Sole precedence
 * source — never inline the fallback elsewhere (the /summary by-institution
 * grouping must group on THIS value so a renamed bank groups correctly).
 */
export function effectiveInstitution(item: AccountInstitutionFields): string {
  const override = item.institutionOverride;
  if (typeof override === 'string' && override.trim().length > 0) {
    return override.trim();
  }
