/**
 * User-facing message extraction for mutation errors. Maps the API error
  ...
  if (error instanceof ApiError) {
    switch (error.code) {
      case 'VERSION_CONFLICT':
        return 'This was changed on another device. The latest values were reloaded; try again.';
      case 'ALREADY_EXISTS':
        return 'A budget already exists for this category.';
      case 'NOT_FOUND':
        return 'This item no longer exists. It may have been removed on another device.';
